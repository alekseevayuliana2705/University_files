...
R_RATE = 0.07
BETA_LIST = np.arange(0.3, 0.95, 0.01)
...
def project_to_lorenz(y_interior):
    ...
    # сохраняем порядок децилей: доли дохода должны быть неубывающими
...
def run_for_beta(beta_value):
    ...
    for year0, year1 in all_transition_years:
        ...
        def objective(phi_nodes):
            ...
            y_model_next = project_to_lorenz(y0[1:] + rhs)
            y_obs_next = y1

            sse = np.sum((y_model_next[1:] - y_obs_next[1:]) ** 2)
