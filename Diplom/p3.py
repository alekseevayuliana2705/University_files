...
for year in years_list:
    ...
    x_smooth, y_smooth = smooth_lorenz_curve(shares_pre, n_points=300)
...
print("Число лет для pre-tax кривых Лоренца:", len(years_list))
...
def phi_quad_from_params(gamma, params, beta_model):
    a, p1, p2 = params
    b = np.exp(p1)
    d = np.exp(p2)
...
def initial_quad_params():
    return np.array([-0.3, 0.0, -1.0])


def window_objective_quad(params, window):
    total_sse = 0.0
    ...
    for year0, year1 in window:
        ...
        phi_vals = phi_quad_from_params(gamma0, params, BETA_MODEL)
...
# ==========================================================
# 5. phi ПО ОКНАМ (квадратично-экспоненциальная)
# ==========================================================
...
for window in window_list:
    ...
    phi_dense = phi_quad_from_params(gamma_dense_phi, params_opt, BETA_MODEL)
...
plt.tight_layout()
plt.savefig("phi_quad.eps", format="eps", bbox_inches="tight")
plt.show()
...
# ==========================================================
# 7. АППРОКСИМАЦИЯ СРЕДНЕЙ phi* ДВУМЯ ПАРАМЕТРИЗАЦИЯМИ
# ==========================================================

# ---- квадратично-экспоненциальная ----
def phi_quad_parametric(gamma, params):
    a, p1, p2 = params
    b = np.exp(p1)
    d = np.exp(p2)
    return BETA_MODEL + np.exp(a - b * gamma - d * gamma**2)

def fit_objective_quad(params):
    phi_fit = phi_quad_parametric(common_gamma, params)
    return np.mean((phi_fit - phi_mean)**2)

res_quad = minimize(fit_objective_quad, initial_quad_params(), method="L-BFGS-B")
params_star_quad = res_quad.x

a_star_quad = params_star_quad[0]
b_star_quad = np.exp(params_star_quad[1])
d_star_quad = np.exp(params_star_quad[2])

phi_star_quad = phi_quad_parametric(common_gamma, params_star_quad)

print("\n==============================")
print("ПАРАМЕТРЫ СРЕДНЕЙ phi* (квадратично-экспоненциальная)")
print("==============================")
print(f"a = {a_star_quad:.6f}")
print(f"b = {b_star_quad:.6f}")
print(f"d = {d_star_quad:.6f}")

print("\nФОРМУЛА phi*_quad(gamma):")
print(
    f"phi*_quad(γ) = {BETA_MODEL:.4f} + exp("
    f"{a_star_quad:.4f} - {b_star_quad:.4f}γ - {d_star_quad:.4f}γ²)"
)

# ---- экспоненциальная ----
def phi_exp_parametric(gamma, params):
    a, p = params
    c = np.exp(p)
    return BETA_MODEL + np.exp(a - c * gamma)

def initial_exp_params():
    return np.array([-0.3, 0.0])

def fit_objective_exp(params):
    phi_fit = phi_exp_parametric(common_gamma, params)
    return np.mean((phi_fit - phi_mean)**2)

res_exp = minimize(fit_objective_exp, initial_exp_params(), method="L-BFGS-B")
params_star_exp = res_exp.x

a_star_exp = params_star_exp[0]
c_star_exp = np.exp(params_star_exp[1])

phi_star_exp = phi_exp_parametric(common_gamma, params_star_exp)

print("\n==============================")
print("ПАРАМЕТРЫ СРЕДНЕЙ phi* (экспоненциальная)")
print("==============================")
print(f"a = {a_star_exp:.6f}")
print(f"c = {c_star_exp:.6f}")

print("\nФОРМУЛА phi*_exp(gamma):")
print(
    f"phi*_exp(γ) = {BETA_MODEL:.4f} + exp("
    f"{a_star_exp:.4f} - {c_star_exp:.4f}γ)"
)

# ---- сравнение на одном графике ----
plt.figure(figsize=(9, 6))
plt.plot(common_gamma, phi_star_quad, linewidth=3, label="Квадратично-экспоненциальная")
plt.plot(common_gamma, phi_star_exp, "--", linewidth=3, label="Экспоненциальная")
plt.axhline(BETA_MODEL, linestyle="--", color="black", linewidth=1.2, label=r"$\beta$")
plt.title(r"Сравнение итоговых функций $\varphi^*(\gamma)$")
plt.xlabel(r"$\gamma$")
plt.ylabel(r"$\varphi(\gamma)$")
plt.grid(alpha=0.3)
plt.legend()
plt.tight_layout()
plt.savefig("phi_comparison.eps", format="eps", bbox_inches="tight")
plt.show()


# ==========================================================
# 8. ДИНАМИКА С phi* (квадратично-экспоненциальная)
# ==========================================================

def phi_star_func(gamma):
    return phi_quad_parametric(gamma, params_star_quad)
...
print("\n==============================")
print("ФУНКЦИОНАЛЫ ДЛЯ phi* (квадратично-экспоненциальная)")
print("==============================")
