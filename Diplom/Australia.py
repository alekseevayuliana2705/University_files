...
FILE_PATH = "/Users/ulianaalekseeva/PyCharmMiscProject/australia_income_merged_filled.csv"
...
plt.title(f"Lorenz curves ({START_YEAR}–{END_YEAR})")
plt.xlabel(r"$x$")
plt.ylabel(r"$y(x,t)$")
plt.grid(alpha=0.3)
...
plt.title(r"Function $\psi_0(\gamma)$")
plt.xlabel(r"$\gamma$")
...
plt.title(r"Annual functions $\phi(\gamma)$ and aggregated $\phi^*(\gamma)$")
plt.xlabel(r"$\gamma$")
...
plt.title(r"Dynamics of $\phi(\gamma)$ by periods")
plt.xlabel(r"$\gamma$")
plt.ylabel(r"$\phi(\gamma)$")
plt.grid(alpha=0.3)
plt.legend()
...
# ==========================================================
# 9. APPROXIMATION OF phi* BY SUM OF LOGISTIC FUNCTIONS
# phi(gamma) = beta + sum_i A_i / (1 + exp(c_i * (gamma - d_i)))
# ==========================================================

N_LOGISTIC = 3

def phi_multi_logistic(gamma, params):
    gamma = np.asarray(gamma, dtype=float)
    phi = np.full_like(gamma, BETA_MODEL, dtype=float)
...
def approximation_objective(params):
    ...
    phi_fit = phi_multi_logistic(common_gamma, params)

    if np.any(~np.isfinite(phi_fit)):
        return 1e12

    # штраф за нарушение phi > beta
    if np.any(phi_fit <= BETA_MODEL):
        return 1e12

    mse = np.mean((phi_fit - phi_star_vals) ** 2)

    # на всякий случай штрафуем нарушение убывания
    diff = np.diff(phi_fit)
    monotone_penalty = 1e6 * np.sum(np.maximum(diff, 0.0) ** 2)
...
# каждая логистика отвечает за свой участок gamma
x0 = np.array([
    0.35 * A0, 8.0, 0.6,
    0.35 * A0, 3.0, 1.6,
    0.30 * A0, 1.2, 2.8
...
for i in range(N_LOGISTIC):
    bounds.extend([
        (1e-8, 10.0),                         # A_i
        (1e-8, 30.0),                         # c_i
        (common_gamma.min(), common_gamma.max()) # d_i
    ])
...
phi_star_approx_vals = phi_multi_logistic(common_gamma, params_approx)

MSE_approx = np.mean((phi_star_approx_vals - phi_star_vals) ** 2)

print("\n==============================")
print("APPROXIMATION OF phi* BY SUM OF LOGISTIC FUNCTIONS")
print("==============================")
...
for i in range(N_LOGISTIC):
    A = params_approx[3 * i]
    c = params_approx[3 * i + 1]
    d = params_approx[3 * i + 2]
    print(f"A{i+1} = {A:.8f}, c{i+1} = {c:.8f}, d{i+1} = {d:.8f}")

print("\nExplicit formula:")
formula = f"phi*(gamma) = {BETA_MODEL:.6f}"
for i in range(N_LOGISTIC):
    A = params_approx[3 * i]
    c = params_approx[3 * i + 1]
    d = params_approx[3 * i + 2]
    formula += f" + {A:.8f}/(1 + exp({c:.8f}*(gamma - {d:.8f})))"
print(formula)


def phi_star_approx(gamma):
    gamma = np.asarray(gamma, dtype=float)
    return phi_multi_logistic(gamma, params_approx)
...
plt.plot(
    ...
    color="red",
    linewidth=3,
    label=rf"Logistic approximation, MSE={MSE_approx:.2e}"
)
...
plt.title(r"Approximation of $\phi^*(\gamma)$ by a sum of logistic functions")
plt.xlabel(r"$\gamma$")
plt.ylabel(r"$\phi(\gamma)$")
plt.grid(alpha=0.3)
plt.legend()
...
def compute_functionals(phi_function):
    ...
    for year0, year1 in all_transition_years:
        ...
        sum_y_obs_one += np.sum(y1[1:])
        n_one += len(y1[1:])

    MSE_one = J_one / n_one
    RMSE_one = np.sqrt(MSE_one)
    Q_one = np.sqrt(J_one) / sum_y_obs_one
    ...
    for year in sorted(y_model.keys()):
        ...
        J_traj += err
        sum_y_obs_traj += np.sum(y_obs[1:])
        n_traj += len(y_obs[1:])

        years_error.append(year)
        errors.append(err / len(y_obs[1:]))
        normalized_errors.append(np.sqrt(err) / np.sum(y_obs[1:]))

    MSE_traj = J_traj / n_traj
    RMSE_traj = np.sqrt(MSE_traj)
    Q_traj = np.sqrt(J_traj) / sum_y_obs_traj
    ...
    return {
        ...
        "sum_y_obs_one": sum_y_obs_one,
        "sum_y_obs_traj": sum_y_obs_traj,
...
plt.title("Observed vs model Lorenz curves")
plt.xlabel("x")
plt.ylabel("y")
plt.grid(alpha=0.3)
