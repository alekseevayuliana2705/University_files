...
WITH wnd_st AS (
    SELECT
        ...
        COUNT(*) FILTER (
            ...
            AND ABS(EXTRACT(EPOCH FROM (t.dt_nd - z.dt_nd_p))) > 1200
            AND z.flg_wnd = 1 
        ) AS total_wnd,
        COUNT(*) FILTER (
            WHERE z.status_fact = 2 
...
colors1 = ["#1CD000", "#FFE200","#FF7100"]
colors2 = ["#FFE200", "#FF7100"]
colors3 = ["#FF9B00", "#FF0900"]
...
metrics = [
    ('delayed_less_1h_pct', 'от 20 минут до 1 часа', cmap1),
    ('delayed_1_3h_pct', 'от 1 часа до 3 часов', cmap2),
    ('delayed_over_3h_pct', 'от 3 часов', cmap3)
]
...
plt.suptitle('Анализ фактов отклонений по времени начала предоставления «окон»\nот плановых параметров по месяцам и годам за 2005 - 2020 г.',
            fontsize=18, y=0.975)
plt.tight_layout()
plt.show()